...
const Main = ({
  ...
  useEffect(() => {
    const run = async () => {
      let igaxBotChatId = localStorage.getItem("igax_bot_chat_id");
      if (
        isMasterTab &&
        isSynced &&
        currentUserId && // user is logged in
        !igaxBotChatId
      ) {
        openChatByUsername({ username: 'igax_bot' });
        
        const currentChat = selectCurrentMessageList(getGlobal());
        console.log("currentChat", currentChat);
        if (currentChat?.chatId) {
          // Save chat ID to cache
          localStorage.setItem('igax_bot_chat_id', currentChat.chatId);
        }
      }
      igaxBotChatId = localStorage.getItem("igax_bot_chat_id");
      if (igaxBotChatId) {
        const pinnedChatIds = getGlobal().chats.orderedPinnedIds.active
        if (pinnedChatIds && !pinnedChatIds.includes(igaxBotChatId)) {
          await toggleChatPinned({ id: igaxBotChatId, folderId: 0 });
        }
      }
    };
    run();
  }, [isMasterTab, isSynced, currentUserId, openChatByUsername]);
