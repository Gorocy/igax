...
import { isBlinkUrl } from '../../../util/blinkUtils';
...
import BlinkWebPage from '../../blinks/BlinkWebPage';
...
const WEBPAGE_BLINK_TYPE = 'telegram_blink';
...
const WebPage: FC<OwnProps & StateProps> = ({
  ...
  const isBlink = type === WEBPAGE_BLINK_TYPE || isBlinkUrl(url);
  
  // For blink URLs, we need a simple className
  if (isBlink) {
    const blinkClassName = buildClassName('WebPage', inPreview && 'in-preview');
    return <BlinkWebPage webPage={webPage} className={blinkClassName} />;
  }
